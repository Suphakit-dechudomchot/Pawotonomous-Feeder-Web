...
import { supabaseClient } from './firebaseConfig.js';

export async function sendCommand(command, payload = {}) {
    if (!DOMElements.deviceStatusCircle || !DOMElements.deviceStatusCircle.classList.contains('online') || !state.isAuthReady) { await showCustomAlert('ออฟไลน์','อุปกรณ์ไม่ได้เชื่อมต่อ หรือการยืนยันตัวตนยังไม่พร้อม. ไม่สามารถส่งคำสั่งได้','error'); return false; }
    try {
...
export async function feedNow() {
    ...
    try {
        // read calibration
        // for simplicity, using state.gramsPerSecond
        ...
        let audioUrl = null; let originalNoiseFileName = null;
        if (audioFile) {
            const path = `quick_feed_noises/${state.currentDeviceId}/${Date.now()}_${sanitizeFileName(audioFile.name)}`;
            try { const { error } = await supabaseClient.storage.from('audio').upload(path, audioFile, { upsert: true }); if (error) throw error; const { data: publicData } = supabaseClient.storage.from('audio').getPublicUrl(path); audioUrl = publicData.publicUrl; originalNoiseFileName = audioFile.name; } catch (e) { await showCustomAlert('อัปโหลดเสียงล้มเหลว', `ไม่สามารถอัปโหลดไฟล์เสียงได้: ${e.message}`, 'error'); setButtonState(DOMElements.feedNowBtn, false); return; }
        }
        ...
        if (await sendCommand('feedNow', { duration_seconds: durationSeconds.toFixed(2), fanStrength, fanDirection, swingMode, noiseFile: audioUrl, originalNoiseFileName })) {
    ...
    } catch (error) { console.error('Error in feedNow:', error); await showCustomAlert('ข้อผิดพลาด', `ไม่สามารถส่งคำสั่งให้อาหารได้: ${error.message}`, 'error'); }
